...
class StateMachine(metaclass=ABCMeta):
    ...
    def __init__(self, initial_state, priority_steps=10):
    ...
    def do_action(self, event_id: EVENT_VAR, *args, **kwargs):
        ...
        for priority_step in range(self.priority_steps):
            for step in range(
    ...
    def make_action_dict(self):
        ...
        for action in type(self).ACTION_VAR:
            ...
            for _ in range(self.priority_steps):
    ...
    def add_common_act(self, func, pos=-1):
        self.common_action_list.insert(pos, func)

    def add_act(self, action: ACTION_VAR, step, func):
        self.action_dict[action.name][step].append(func)
...
class SM_KICS(StateMachine):
    ...
    class EVENT_VAR(Enum):
        ...
        EVENT_TOTAL = auto()  # dummy

    def __init__(self):
        super().__init__(type(self).STATE_VAR.KINMU_MAE, priority_steps=6)
        self.NEXT_STATE_TBL = (
...
def test():
    ...
    # print(vars(test))
    test.add_common_act(test_action)
