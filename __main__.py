...
class ButtonElement(EventManage):
    def __init__(self, frame, text, event_id:EVENT_VAR, button_id:BUTTON_ID, state=tk.NORMAL):
        ...
        self.b_element = tk.Button(master=frame, textvariable=self.text, font=self.font, command=self.click, state=state)
    def click(self):
...
class TimeLine():
    ...
    def log_stamp(self, obj:ButtonElement):
        MSG_TABLE = [["勤務開始", "勤務終了", "休憩終了"],
                     ["？",      "休憩開始",  "?"]]
        date = str(gyomu.timelist[-1].month) +'/'+ str(gyomu.timelist[-1].day)
        time = str(gyomu.timelist[-1].hour) +':'+ str(gyomu.timelist[-1].minute)
        log_msg = date +' '+ time +' '+ MSG_TABLE[obj.event_id.value][EventManage.state_var.value] + '\n'
...
def update_button():
    for button in b_array:
        button.text.set(BUTTON_TBL[button.button_id.value][EventManage.new_state.value][0])
        button.b_element.config(state=BUTTON_TBL[button.button_id.value][EventManage.new_state.value][1])
...
if __name__=="__main__":
    ...
    state_label = tk.Label(master=window, text="state", font=("MSゴシック", "20", "bold"))

    id_form   = InputForm(master=window, text="DSC-ID",   font=default_font)
    pass_form = InputForm(master=window, text="Password", font=default_font, show="*")

    b1 = ButtonElement(frame=window, text="勤務開始", button_id=BUTTON_ID.KINMU_BUTTON, event_id=EVENT_VAR.KINMU_BUTTON_PUSHED)
    b2 = ButtonElement(frame=window, text="休憩開始", button_id=BUTTON_ID.KYUKEI_BUTTON, event_id=EVENT_VAR.KYUKEI_BUTTON_PUSHED,state=tk.DISABLED)

    #このへんもっといい実装方法ありそう
    b_array=(b1, b2)

    log = scrolledtext.ScrolledText(master=window, width=20, height=10, bd=5, state='disabled')
